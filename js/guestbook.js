document.addEventListener("DOMContentLoaded", function () {
  ...
  async function fetchEntries() {
    try {
      const response = await axios.get("http://35.173.37.0:8000/api/entries");
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      const response = await axios.post("http://35.173.37.0:8000/api/entries", {
        name,
        message,
      });
    ...
    if (e.target.tagName === "BUTTON") {
      ...
      try {
        const response = await axios.delete(
          `http://35.173.37.0:8000/api/entries/${entryId}`
        );
